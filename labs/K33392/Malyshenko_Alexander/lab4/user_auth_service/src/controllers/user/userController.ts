...
import User, { UserCreate, UserUpdatePassword } from "../../models/user/User"
...
class UserController {
    ...
    patchUpdateUserPassword = async (request: Request, response: Response) => {
        let passwordData: UserUpdatePassword = request.body

        const current_user = await this.userService.findByEmail(passwordData.email)
        if (!current_user) {
            return response.status(404).send()
        }

        const isPasswordCorrect =
            await this.passwordHandler.comparePassword(passwordData.password, current_user.password)
        ...
        const data: UserCreate = {
            password: await this.passwordHandler.hashPassword(passwordData.new_password),
            email: passwordData.email,
        }

        try {
            await this.userService.updateById(current_user.id, data)
